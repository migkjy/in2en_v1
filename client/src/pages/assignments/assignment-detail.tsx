...
export default function AssignmentDetail() {
  ...
  const { data: assignment, isLoading: isAssignmentLoading } = useQuery<Assignment & { class?: Class; branch?: Branch }>({
    queryKey: ["/api/assignments", assignmentId],
    queryFn: async () => {
      const response = await fetch(`/api/assignments/${assignmentId}`);
      if (!response.ok) {
  ...
  // Query to fetch submissions for this assignment
  ...
  // Query to fetch students for the class
  const { data: students = [] } = useQuery<User[]>({
    queryKey: ["/api/classes", assignment?.classId, "students"],
    queryFn: async () => {
    ...
    enabled: !!assignment?.classId && user?.role !== "STUDENT",
  });
  ...
  const aiReviewMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Success",
        description: "AI review process started. This may take a few moments.",
      });
    },
    onError: (error) => {
      toast({
  ...
  // Get filtered submissions based on user role
      ...
      <main className="flex-1 p-8 overflow-auto">
        <div className="max-w-6xl mx-auto">
          <div className="mb-6">
            <Button
              variant="outline"
              onClick={() => navigate(user?.role === "STUDENT" ? "/student/assignments" : `/${params?.role}/assignments`)}
              className="mb-2"
            >
              <ArrowLeft className="w-4 h-4 mr-2" />
              Back to Assignments
                  ...
                  <div>
                    ...
                    {isTeacherOrAdmin && submissions.length > 0 && (
                      <div className="mt-6">
                        ...
                        <Button
                          variant="outline"
                          onClick={() => navigate(`${basePath}/assignments/upload`)}
                        >
                          Upload Homework
                        </Button>
                      </div>
                    ...
                    {user?.role === "STUDENT" && (
                      <div className="mt-6">
                        <Button
                          onClick={() => navigate(`/student/assignments/${assignmentId}/upload`)}
                        >
                          Upload Homework
                        </Button>
                      </div>
                    ...
                    <TableBody>
                      {filteredSubmissions.map((submission) => (
                        <TableRow key={submission.id}>
                          {isTeacherOrAdmin && (
                            <TableCell>
                              {studentMap.get(submission.studentId) || `Student ${submission.studentId}`}
                            </TableCell>
                          )}
                          <TableCell>
