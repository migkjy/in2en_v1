...
export default function UploadAssignment() {
  ...
  const renderContent = () => (
    <div className={isMobile ? "p-4" : "max-w-4xl mx-auto"}>
                ...
                {files.map((file) => (
                  ...
                  >
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="absolute top-1 right-1 h-8 w-8 p-1"
  ...
  if (isMobile) {
    return (
      <MobileLayout 
        title={`Upload: ${assignment?.title || 'Homework'}`}
        backPath={`/${basePath}/assignments/${assignmentId}`}
      >
        {renderContent()}
      </MobileLayout>
    );
